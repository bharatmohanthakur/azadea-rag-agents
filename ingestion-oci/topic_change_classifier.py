"""
Topic-change & ambiguity classifier — single LLM call gate.
...
If either is true, produces a clarifying question to ask the user.

Outcome-oriented prompt: no phrase lists, no taxonomies, no thresholds.
The same model (google.gemini-2.5-flash via OCI) decides everything.
"""
...
class TopicChangeResult(BaseModel):
    ...
    is_ambiguous: bool = Field(
        description="True if the current query is too short or vague to be sure what the user means without more context."
    )
    ...
    suggested_question: str = Field(
        default="",
        description="If topic_changed or is_ambiguous is true, write ONE short friendly clarifying question. Otherwise empty.",
    )
    reasoning: str = Field(
        default="",
...
1. topic_changed — Has the user moved to a different topic than what they were
   asking about? Consider the dominant topic in recent queries; a brief
   detour doesn't count if they're now back on the main thread.
...
2. is_ambiguous — Is the current query too short, vague, or context-dependent
   to be confident what the user actually means without asking? Short pronoun
   or fragment messages ("first action?", "the formula", "and another thing")
...
If both flags are false, set suggested_question to empty string.
