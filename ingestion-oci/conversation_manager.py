...
MAX_REDIS_CONNECTIONS = int(os.getenv("REDIS_MAX_CONNECTIONS", "50"))
...
class ConversationManager:
    ...
    Stress-test safe:
      - Connection pool (50 connections, not 1)
      - Atomic add_message via Redis pipeline (no race conditions)
    ...
    def __init__(self):
        ...
        if REDIS_AVAILABLE:
            try:
                ...
                logger.info(f"✅ Connected to Redis at {REDIS_HOST}:{REDIS_PORT} (pool={MAX_REDIS_CONNECTIONS})")
    ...
    def add_message(self, user_id: str, role: str, content: str, metadata: Optional[Dict[str, Any]] = None):
        ...
        Uses Redis optimistic locking (WATCH/MULTI) to prevent race conditions
        when two requests for the same user arrive simultaneously.
        """
        message = {
            "role": role,
            "content": content,
        ...
        try:
            if self.redis_client:
                key = self._get_key(user_id)
                ttl = int(timedelta(days=CONVERSATION_TTL_DAYS).total_seconds())

                # Atomic read-modify-write with optimistic locking
                # Retries if another client modified the key between WATCH and EXEC
                for attempt in range(3):
                    try:
                        pipe = self.redis_client.pipeline(True)  # True = transactional
                        pipe.watch(key)
                        ...
                        pipe.multi()
                        pipe.setex(key, ttl, json.dumps(history, ensure_ascii=False))
                        pipe.execute()
                        return  # Success
                ...
                # All retries failed — fallback to non-atomic (better than losing the message)
                logger.warning(f"Optimistic lock failed for {user_id}, using non-atomic write")
                data = self.redis_client.get(key)
                history = json.loads(data) if data else []
                history.append(message)
                if len(history) > MAX_HISTORY_SIZE:
                    history = history[-MAX_HISTORY_SIZE:]
                self.redis_client.setex(key, ttl, json.dumps(history, ensure_ascii=False))
            else:
                if user_id not in self.memory_fallback:
                    self.memory_fallback[user_id] = []
                self.memory_fallback[user_id].append(message)
                if len(self.memory_fallback[user_id]) > MAX_HISTORY_SIZE:
                    self.memory_fallback[user_id] = self.memory_fallback[user_id][-MAX_HISTORY_SIZE:]
        except Exception as e:
