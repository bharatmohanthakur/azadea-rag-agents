...
def upsert_typed_chunks(
    ...
    """
    Upsert typed chunks with full payload metadata.
    
    Each chunk_payload should contain:
        - chunk_id: unique identifier
        - chunk_type: type of chunk (image_description, control, etc.)
        - text: the chunk text content
        - Plus all other metadata fields
    """
    ...
    print(f"[qdrant] upserted {len(points)} typed chunks → {collection} ({type_summary})")
...
def hybrid_search(
    ...
    for p in res.points:
        ...
        hits.append({
            ...
            # New typed chunk fields
            "chunk_type": pl.get("chunk_type"),
            "domain": pl.get("domain"),
            "function": pl.get("function"),
            "variant": pl.get("variant"),
            "page": pl.get("page"),
            "figure_type": pl.get("figure_type"),
            "roles": pl.get("roles", []),
...
def hybrid_search_typed(
    ...
    """
    Hybrid search with chunk type and metadata filtering.
    
    Args:
        chunk_types: Filter to specific chunk types (e.g., ['control', 'definition'])
        domain: Filter to specific domain (e.g., 'ABS', 'ACC')
        function: Filter to specific function (e.g., 'SPD', 'CNA')
    """
    ...
    if chunk_types:
        conditions.append(
            qm.FieldCondition(
                key="chunk_type",
                match=qm.MatchAny(any=chunk_types)
            )
        )
    ...
    if domain:
        conditions.append(
            qm.FieldCondition(
                key="domain",
                match=qm.MatchValue(value=domain)
            )
        )
    ...
    if function:
        conditions.append(
            qm.FieldCondition(
                key="function",
                match=qm.MatchValue(value=function)
            )
        )
    
    where = qm.Filter(must=conditions) if conditions else None
    
    return hybrid_search(client, collection, query_text, top_k, prefetch_k, where)
