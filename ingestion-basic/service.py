#!/usr/bin/env python3
"""
Document Ingestion Service — BASIC TIER (Port 8071)
AWS Textract-style async job pattern: submit → poll → get result.

Pipeline:
  1. PDF → Markdown via Azure Document Intelligence (OCR only, NO figures)
  2. Semantic chunking (embedding-similarity boundaries, NO LLM grouping)
  3. Hybrid vectors (dense + sparse BM25) → Qdrant

No GPT-4V figure extraction, no GPT-4o page grouping.
...
# ---------------------------------------------------------------------------
# Paths — pipeline modules are at /app/pipeline inside Docker
# ---------------------------------------------------------------------------
...
MD_OUT_DIR   = Path(os.getenv("MD_OUT_DIR", str(PIPELINE_DIR / "md_out_data_basic"))).resolve()
...
from ingest_data_folder import process_single_pdf
...
# ---------------------------------------------------------------------------
TIER           = "basic"
COLLECTION     = os.getenv("QDRANT_COLLECTION", "docs_basic_azadea")
QDRANT_URL     = os.getenv("QDRANT_URL", "http://localhost:6333")
QDRANT_API_KEY = os.getenv("QDRANT_API_KEY")
PORT           = int(os.getenv("SERVICE_PORT", "8071"))
JOB_TTL_HOURS  = 24
MAX_JOBS_LIST  = 100
...
logger = logging.getLogger("ingestion_basic")
...
async def lifespan(app: FastAPI):
    global _doc_client, _qdrant_client

    logger.info(f"Starting BASIC ingestion service on port {PORT}...")
    ...
    logger.info(f"BASIC tier ready. Collection '{COLLECTION}'.")
    yield
...
app = FastAPI(
    title="Document Ingestion — Basic Tier",
    description="Azure DI OCR → semantic chunking → Qdrant hybrid vectors. No images, no LLM grouping.",
    version="1.0.0",
    lifespan=lifespan,
)
...
def run_pdf_to_md(pdf_path: Path) -> Path:
    """Basic: Azure DI OCR only — no figure extraction."""
    MD_OUT_DIR.mkdir(parents=True, exist_ok=True)
    result_path = process_single_pdf(
        (pdf_path, MD_OUT_DIR, _doc_client, 1, 1)
    )
    if result_path is None:
        raise RuntimeError(f"Azure DI conversion failed for '{pdf_path.name}'")
    return result_path


def run_md_to_qdrant(md_path: Path) -> int:
    """Basic: semantic chunking (embedding similarity) → hybrid upsert."""
    text = md_path.read_text(encoding="utf-8", errors="ignore")
    chunks = qdrant_ingest.semantic_chunks(text, qdrant_ingest.MAX_TOKENS, qdrant_ingest.MIN_CHUNK_SIZE)
    if not chunks:
        raise RuntimeError(f"No chunks produced from '{md_path.name}'")
    ...
    qdrant_ingest.upsert_hybrid_points(
        _qdrant(), COLLECTION, chunks, meta, dense_vecs, sparse_vecs
    )
    return len(chunks)
    ...
    async with _get_doc_lock(job.doc_id):
        try:
            ...
            if job.operation == "delete":
                ...
                # PDF → MD (basic OCR only)
                job.step = "pdf_to_md"
                job.progress = "Converting PDF → Markdown via Azure Document Intelligence (OCR only)..."
                try:
                ...
                # MD → Qdrant (semantic chunking)
                job.step = "md_to_qdrant"
                job.progress = "Semantic chunking → embedding → Qdrant hybrid upsert..."
                try:
                ...
                job.progress = f"{op_label} {chunks} chunks (semantic, no images)."
