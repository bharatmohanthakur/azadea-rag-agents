...
# NOTE: group_id is now derived from user_id per request (not a static global)
...
async def search_graphiti_memory(query: str, user_id: str = "default_user", num_results: int = 5) -> List[Dict[str, Any]]:
    """
    Search the Graphiti knowledge graph for relevant facts.

    Per official docs: group_ids=[user_id] for per-user memory isolation.
    No marker-tag classification — Graphiti handles entity extraction natively.

    Args:
        query: Search query
        user_id: User identifier used as group_id for memory isolation
    ...
    try:
        ...
        results = await circuit.acall(
            ...
            group_ids=[user_id],  # Per-user isolation (not static "azadea")
        )
        ...
        logger.info(f"🧠 Graphiti search returned {len(facts)} facts (group_id={user_id})")

        if facts:
            for i, fact in enumerate(facts[:3], 1):
                logger.info(f"  Fact {i}: {fact.get('fact', '')[:200]}...")
...
class GraphitiContextCache:
    ...
    # Legacy compatibility — these now all return the full fact list
    # since we no longer classify by marker tags
    @property
...
async def prefetch_graphiti_context(query: str, user_id: str, num_results: int = 25) -> GraphitiContextCache:
    ...
    Per official docs: query is just the search query (not concatenated with user_id).
    User isolation is handled by group_ids=[user_id] inside search_graphiti_memory.
    """
    ...
    logger.info(f"📦 Prefetched {len(all_facts)} Graphiti facts for user={user_id}")
    return GraphitiContextCache(all_facts)


async def save_to_graphiti_memory(user_id: str, query: str, answer: str) -> bool:
    """
    Save a conversation interaction to Graphiti as a clean episode.

    Per official Graphiti docs:
    - Clean natural-language body (no marker tags) so entity extraction works properly
    - group_id = user_id for per-user memory isolation
    - EpisodeType.message for conversations
    - Full untruncated content preserved
    """
    graphiti = await get_graphiti()
    if not graphiti:
        return False
    ...
    try:
        # Clean episode body — let Graphiti's LLM extract entities naturally
        episode_content = f"User asked: {query}\n\nAssistant answered: {answer}"
        ...
        await graphiti.add_episode(
            ...
            group_id=user_id,  # Per-user isolation (not static "azadea")
        )
        elapsed_sec = (datetime.now() - start_time).total_seconds()
        _update_graphiti_trace("save", elapsed_sec)
        logger.info(f"💾 Saved conversation to Graphiti (group_id={user_id}, {elapsed_sec:.2f}s)")
        return True
    except Exception as e:
...
async def enhance_query_with_graphiti_context(
    ...
    """
    Enhance query with Graphiti memory context.

    Simplified: single prefetch, no parallel sub-helpers, no temporal_flow (was never used in prompts).
    """
    try:
        ...
        t1 = time_module.time()
        logger.info(f"⏱️ TIMING: Graphiti prefetch took {(t1-t0)*1000:.0f}ms, {len(graphiti_cache)} facts")

        # Build context directly from prefetched facts
        related_conversations = graphiti_cache.all_facts[:5]
        ...
        enhanced_context = {
            ...
            "_graphiti_cache": graphiti_cache,
        }

        logger.info(f"🚀 Enhanced query with Graphiti: {len(related_conversations)} related facts")
...
async def query_backup_endpoint(request: QueryRequest):
    ...
    try:
        ...
        graphiti_facts = await search_graphiti_memory(search_query, user_id=user_id, num_results=5)
        ...
        # 5. Save to Graphiti Memory (single clean episode, async with error handling)
        async def _background_save():
            try:
                await save_to_graphiti_memory(user_id, query_text, answer_text)
            except Exception as e:
                logger.error(f"⚠️ Background Graphiti save failed: {e}")
        ...
        async def graphiti_search(cache):
            ...
            try:
                if cache is not None:
                    ...
                    facts = await search_graphiti_memory(query, user_id=user_id, num_results=5)
...
class AgentState(TypedDict):
    ...
    # graphiti_temporal_flow removed — was computed but never used in LLM prompts
...
async def query_endpoint(request: QueryRequest):
    ...
    try:
        ...
        logger.info(f"🚀 Graphiti context: profile={graphiti_context.get('context_summary', {}).get('has_profile', False)}, "
                   f"related={graphiti_context.get('context_summary', {}).get('related_conversation_count', 0)}")
        ...
        # Save to Graphiti Memory (single clean episode, async with error handling)
        async def _background_save():
            try:
                await save_to_graphiti_memory(user_id, query_text, answer_text)
            except Exception as e:
