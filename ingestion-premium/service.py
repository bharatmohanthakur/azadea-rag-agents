#!/usr/bin/env python3
"""
Document Ingestion Service — PREMIUM TIER (Port 8073)
...
Pipeline:
  ...
  3. LLM-guided page grouping (GPT-4o groups pages by topic)
...
Full multimodal + LLM page grouping. Best quality, highest cost.
"""
...
MD_OUT_DIR   = Path(os.getenv("MD_OUT_DIR", str(PIPELINE_DIR / "md_out_data_premium"))).resolve()
...
from ingest_data_folder_multimodal import process_single_pdf_multimodal
from llm_semantic_chunker import (
...
# ---------------------------------------------------------------------------
TIER           = "premium"
COLLECTION     = os.getenv("QDRANT_COLLECTION", "docs_premium_azadea")  # isolated; legacy LLM_COLLECTION (docs_llm_chunked_azadea) is the prod read-only corpus
QDRANT_URL     = os.getenv("QDRANT_URL", "http://localhost:6333")
QDRANT_API_KEY = os.getenv("QDRANT_API_KEY")
PORT           = int(os.getenv("SERVICE_PORT", "8073"))
JOB_TTL_HOURS  = 24
MAX_JOBS_LIST  = 100
...
logger = logging.getLogger("ingestion_premium")
...
async def lifespan(app: FastAPI):
    ...
    logger.info(f"Starting PREMIUM ingestion service on port {PORT}...")
    ...
    logger.info(f"PREMIUM tier ready. Collection '{COLLECTION}'.")
    yield
...
app = FastAPI(
    title="Document Ingestion — Premium Tier",
    description=(
        "Azure DI OCR + GPT-4V figures + LLM page grouping (GPT-4o) → Qdrant. "
        "7 typed chunks with page_group metadata. Best quality."
    ),
    version="1.0.0",
    lifespan=lifespan,
)
...
def run_pdf_to_md(pdf_path: Path) -> Path:
    """Premium: Azure DI OCR + GPT-4V figure extraction."""
...
def run_md_to_qdrant(md_path: Path) -> int:
    """Premium: LLM page grouping (GPT-4o) → typed chunks → hybrid upsert."""
    ...
    async with _get_doc_lock(job.doc_id):
        try:
            ...
            if job.operation == "delete":
                ...
                # PDF → MD (multimodal with GPT-4V figures)
                job.step = "pdf_to_md"
                ...
                # MD → Qdrant (LLM page grouping)
                job.step = "md_to_qdrant"
                job.progress = "LLM page grouping (GPT-4o) → embedding → Qdrant hybrid upsert..."
                try:
                ...
                job.progress = (
                    f"{op_label} {chunks} LLM-grouped typed chunks "
                    f"(image_description, ocr_detail, control, definition, "
                    f"table_summary, page_context [LLM-grouped], doc_summary)."
                )
